...
$(document).ready(function () {
  ...
  var isCurrentUserOwnCurrentQuestion = false;
        ...
        if (result.success === true) {
          ...
          if (type === 'question') {
            var _result$data2 = result.data,
                is_favorite_by_viewer = _result$data2.is_favorite_by_viewer,
                summary_info = _result$data2.summary_info,
                author_public_id = _result$data2.author_public_id;
            var num_views = summary_info.num_views,
                last_active_date = summary_info.last_active_date;
  ...
  function handleBestAnswer() {
    var textSetBestAnswer = 'Click to set as best answer';
    var textBestAnswer = '✔️ Best Answer';
    var textUnsetBestAnswer = 'Click to unset as best answer';
    $('tvy-content-action-view[data-for="answer"] .setBestAnswer').on('mouseover', function () {
      var text = $(this).hasClass('selected') ? textUnsetBestAnswer : $(this).find('.labelBestAnswer').text();
      $(this).find('.labelBestAnswer').text(text);
    });
    $('tvy-content-action-view[data-for="answer"] .setBestAnswer').on('mouseleave', function () {
      var text = $(this).hasClass('selected') ? textBestAnswer : $(this).find('.labelBestAnswer').text();
      $(this).find('.labelBestAnswer').text(text);
    });
  ...
  function setSummaryInfoOFQuestion(numViews, lastActive) {
    var summaryInfo = $('.questionSummaryInfo');
